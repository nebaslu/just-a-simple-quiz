...
function buildChainStatement(q, claimed) {
  const question = String(q.question || '').trim();
  const body = question.replace(/^¿/, '').replace(/\?$/, '').trim();

  let match = body.match(/^Cuál de los siguientes fue (.+)$/i);
  if (match) {
    return `${match[1]} fue ${claimed}.`;
  }

  match = body.match(/^Cuál de los siguientes NO es (.+)$/i);
  if (match) {
    return `${claimed} no es ${match[1]}.`;
  }

  match = body.match(/^Cuál de los siguientes es (.+)$/i);
  if (match) {
    return `${claimed} es ${match[1]}.`;
  }

  match = body.match(/^(.+?) fue el ex dictador de qué país$/i);
  if (match) {
    return `${match[1]} fue el ex dictador de ${claimed}.`;
  }

  match = body.match(/^Cuál de los siguientes (.+)$/i);
  if (match) {
    return `${claimed} ${match[1]}.`;
  }
...
function buildChainFromClassic(q) {
  ...
  const statement = buildChainStatement(q, claimed);
...
function assignModeData(question) {
...
function pickQuestionsWithThemes(total) {
  ...
  while (result.length < total && categories.length > 0) {
    ...
    if (catQuestions.length > 0) {
      ...
      const questionWithMode = assignModeData(q);
      result.push({
        ...questionWithMode,
        theme: cat,
      });
