...
function buildChainFromClassic(q) {
  ...
  const pickWrong = (index) => wrongs[index % Math.max(1, wrongs.length)] || correct;

  return [
    { text: `En "${q.question}", la respuesta correcta es "${correct}".`, answer: true },
    { text: `En "${q.question}", la respuesta correcta es "${pickWrong(0)}".`, answer: false },
    { text: `La categoria de esta pregunta es "${q.category || 'Mixto'}".`, answer: true },
    { text: `En "${q.question}", la respuesta correcta es "${pickWrong(1)}".`, answer: false },
    { text: `La dificultad marcada es "${q.difficulty || 'media'}".`, answer: true },
  ];
}
...
function assignModeData(question, mode) {
  ...
  if (mode === 'true_false_chain') {
    return {
      ...
      chain: Array.isArray(question.chain) && question.chain.length === 5
        ? question.chain
