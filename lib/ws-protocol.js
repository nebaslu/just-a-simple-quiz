...
function handleRealtimeMessage(ws, req, raw) {
  ...
  if (msg.type === 'spectator:join') {
    joinAsSpectator(ws, msg.room, msg.name, msg.showSelections);
    return;
  }
  ...
  if (msg.type === 'player:rejoin') {
    ...
    if (rejoinPlayer) {
      ...
      if (room.state === 'question' && room.currentQuestion) {
        ...
        send(ws, {
          ...
          serverNow: room.lastRoundResult.serverNow,
          nextQuestionAt: room.lastRoundResult.nextQuestionAt,
          room: roomPublicState(room),
        });
    ...
    if (rejoinSpectator) {
      ...
      send(ws, {
        ...
        role: 'spectator',
        showSelections: rejoinSpectator.showSelections,
        room: roomPublicState(room),
      });
